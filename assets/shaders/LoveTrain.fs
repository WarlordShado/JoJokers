...
extern PRECISION vec2 misfortune;
...
vec4 misfortune_color = vec4(230., 230., 0., 90.) / 255.;
...
bool line(vec2 uv, float offset, float width) {
    ...
    offset = offset + 0.35 * sin(misfortune.x + lines_offset);
    width = width + 0.005 * sin(misfortune.x);
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    pixel = vec4(misfortune_color.rgb * avg + tex.rgb * tex.a, pixel.a);
